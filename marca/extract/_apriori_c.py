...
class AprioriC(Apriori, BaseEstimator, Extract):
    ...
    def __call__(self, X, y):
        ...
        filter_maj = self.B*Counter(y).most_common()[0][1] / len(y)
        class_maj = Counter(y.astype(float)).most_common()[0][0]
        class_min = Counter(y.astype(float)).most_common()[-1][0]

        # Filtro só as regras com consequente majoritário
        self.rules = self.rules.filter(
            lambda x: ((x.support >= filter_maj) and (x.consequent == class_maj)) or (x.consequent == class_min))
