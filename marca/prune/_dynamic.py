...
class DynamicPrune(Prune):
    """
    Dynamic Prune algorithm implementation adapted from:
    Sood, N., Zaiane, O. (2020). Building a Competitive Associative Classifier.
    In: Song, M., Song, IY., Kotsis, G., Tjoa, A.M., Khalil, I. (eds) Big Data
    Analytics and Knowledge Discovery. DaWaK 2020. Lecture Notes in Computer
    Science(), vol 12393. Springer, Cham. https://doi.org/10.1007/978-3-030-59065-9_18
    """
    def __init__(self, rank_method=None, interest_measures=None):
        super().__init__()
        self.name = "DynamicPrune"
        self.rules = None
        self.data = None
    ...
    def __call__(self, X, y, rules):
        ...
        dynamic_rules = CARs.from_numpy(rules.to_numpy(), rules.support, rules.sup_antecedent, rules.sup_consequent,
                                        len(X))

        self.set_cover(data, dynamic_rules)

        while len(data) > 0:
            first_rule = dynamic_rules[0]
            final_classifier.append(first_rule)
            index_covered = first_rule.match_A(data[:, :-1])
            data = np.delete(data, index_covered, axis=0)
            self.delete_cover(dynamic_rules, index_covered)

            if len(data) == 0:
                break

            new_A, new_B, new_AB = self.update_measures_values(data, dynamic_rules)

            rules_remove = np.where(((new_A == 0) | (new_AB == 0)) | (new_B == 0))[0]
            dynamic_rules = dynamic_rules.delete(rules_remove)

            if len(dynamic_rules) == 0:
                break

            # print(len(dynamic_rules))
            ...
            dynamic_rules = self.rank_method(X, y, dynamic_rules, self.interest_measures)
